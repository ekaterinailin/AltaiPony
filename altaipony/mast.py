...
def _query_kepler_products(target, radius=1):
    ...
    try:
        ...
        if (target > 0) and (target < 200000000):
            target_name = 'k2sc{:09d}'.format(target)#changed to original lightkurve
        ...
        obs = Observations.query_criteria(target_name=target_name,
                                          project=["hlsp_k2sc"],#changed to original lightkurve
...
#change to original lightkurve: 'Target Pixel' -> 'Lightcurve' in default kwargs
def search_kepler_products(target, filetype='Lightcurve', cadence='long', quarter=None,
    ...
    qoc = np.asarray([p.split('-')[-1][1:3]
                      for p in products['dataURI']], dtype=int)#change to original lightkurve
    products['qoc'] = qoc
    ...
    if targetlimit is not None:
        ids = np.asarray([p.split('/')[-1].split('-')[0].split('_')[-1]
                          for p in products['dataURI']], dtype=int)#change to original lightkurve
